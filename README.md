...
A JavaScript client for passkey authentication designed to work with Ugarit applications. It provides a fluent API for browser-side WebAuthn ceremonies (registration and authentication).

## Installation

```bash
npm install @ugarit/passkeys
```
...
import { usePasskeyVerify, usePasskeyRegister } from "@ugarit/passkeys/react";
...
<script setup>
import { ref } from "vue";
import { usePasskeyVerify, usePasskeyRegister } from "@ugarit/passkeys/vue";
...
```svelte
<script>
    import { usePasskeyVerify, usePasskeyRegister } from "@ugarit/passkeys/svelte";
...
This package handles communication with your Ugarit application and expects the following endpoints:
...
```jsx
import { PasskeyExistsError } from "@ugarit/passkeys";
...
| `@ugarit/passkeys/svelte` | `usePasskeyVerify`, `usePasskeyRegister` |
